...
- Two env vars exist purely as test seams and are never meant to be set in
  ...
  tables — they're not something the operator ever needs to set. The recap
  modules (`tools/strava-client.mjs`, `tools/road-matching.mjs`,
  `tools/recap-writer.mjs`, `tools/youtube-client.mjs`) follow the same
  seam pattern but as factory-function options (`apiBase`/`oauthBase`/
  `fetchImpl`, defaulting to the real hosts) rather than env vars, since
  they're plain importable modules, not `server.js` — same "don't document
  in README" rule applies.
